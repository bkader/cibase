...
class Theme
{
	...
	public function initialize(array $config = array())
	{
		...
		if ($this->CI->config->item('app.theme'))
		{
			$config['theme.theme'] = $this->CI->config->item('app.theme');
		}

		// Are site_name, site_description and site_keywords overriden?
		foreach (array('name', 'description', 'keywords') as $meta)
		{
			if ($_meta = $this->CI->config->item('app.'.$meta))
			{
				$config['theme.site_'.$meta] = $_meta;
			}
		}
